...
export abstract class PrimitiveToolEx extends PrimitiveTool {
  ...
  public async ensureEditorConnection() {
    if (this._editorConnection !== undefined) return;
...
export class PlacementTestTool extends PrimitiveToolEx {
  ...
  private async createElement(): Promise<void> {
    ...
    return this.editorConnection.createElement(
      props3d,
      origin,
      angles,
      geomprops
  ...
  public async onUndoPreviousStep(): Promise<boolean> {
    ...
    this.points.pop();
    if (0 === this.points.length) this.onReinitialize();
  ...
  public async onKeyTransition(
    ...
    return wentDown && AccuDrawShortcuts.processShortcutKey(keyEvent)
      ? EventHandled.Yes
      : EventHandled.No;
  }
